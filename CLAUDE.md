...
>
> **正本（Source of Truth）は `docs/DESIGN.md`**（アーキテクチャ・セキュリティモデル・将来拡張）。

---

## 1. プロジェクト概要

`batch-scheduler` は Java 21 / Maven 製の小さく堅牢な MVP バッチ実行マネージャ。YAML のジョブ定義を読み、依存 DAG（有向非巡回グラフ）をトポロジカル順で実行する。ジョブごとのリトライ・タイムアウト・環境変数に対応し、失敗ジョブの依存先（dependents）は SKIP、実行履歴は JSON で永続化する。CLI フレームワークは PicoCLI、YAML/JSON のパース・シリアライズは Jackson、テストは JUnit 5。README は日英 2 言語（`README.md` / `README.ja.md`、言語スイッチャ付き）。

ドメイン語彙: **DAG** / **dependsOn**（先行必須ジョブ）/ **retries**（初回失敗後の追加試行回数）/ **timeoutSeconds**（試行ごとのタイムアウト, 0=無制限）/ **JobStatus**（`SUCCEEDED` / `FAILED` / `SKIPPED`）/ **runId**（実行 ID）/ **state directory**（既定 `.batch-state`、実行記録 JSON の保存先）。
...
java -jar target/batch-scheduler.jar validate examples/etl.yaml  # DAG 検証
...
オプション: `--state-dir <dir>`（状態保存先、既定 `.batch-state`）/ `-q`・`--quiet`（ジョブ別サマリ表を抑制）/ `--help` / `--version`。終了コード: 0=成功 / 1=バッチ失敗 / 2=検証エラー / 3=設定・IO エラー。

## 3. アーキテクチャ

### パッケージ構成（`src/main/java/io/github/izumacha/batch/`）

- `cli/` — PicoCLI ベースの CLI。`Main.java`（エントリ）→ `BatchCli.java`（コマンドルータ＋終了コード）、`RunCommand` / `ValidateCommand` / `ListCommand`、出力整形 `CliFormat.java`。
...
```
YAML → BatchConfigLoader.load(path) → Batch(record)
     → DependencyGraph.build(batch)  [検証。全エラーを ValidationException で集約]
     → BatchExecutor.execute(batch)  [topo 順に実行し ExecutionResult を生成]
     → JsonExecutionStore.save(result) [.batch-state/<runId>.json へ書き出し]
```

`examples/etl.yaml` は extract→transform→load→notify のサンプル。テストは `src/test/java/...` に各クラス対応（`BatchConfigLoaderTest` / `BatchExecutorTest` / `DependencyGraphTest` 等）。

### セキュリティ・信頼モデル（`docs/DESIGN.md`）

バッチ定義ファイルは **Makefile / CI パイプラインと同等の信頼入力**として扱う。一方で資源枯渇には防御する: bounded YAML parsing、出力キャプチャの上限、反復的（再帰でない）グラフアルゴリズム、state ディレクトリの安全性（runId 検証・シンボリックリンク非追従）。MVP 非目標（non-goals）: スケジューリング・並列実行・分散。
